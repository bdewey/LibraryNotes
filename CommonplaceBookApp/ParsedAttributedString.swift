...
@objc public final class ParsedAttributedString: NSMutableAttributedString {
  ...
  public override func replaceCharacters(in range: NSRange, with str: String) {
    ...
    if let range = changedAttributesRange {
      Logger.attributedStringLogger.debug("Changed attributes at \(visibleTextRange(forRawRange: NSRange(range)))")
    }
    delegate?.attributedStringDidChange(
      oldRange: range,
      changeInLength: memoizedString.length - lengthBeforeChanges,
      changedAttributesRange: changedAttributesRange.flatMap({ visibleTextRange(forRawRange: NSRange($0)) })
