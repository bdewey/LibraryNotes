...
public final class VocabularyAssociation: ChallengeTemplate, Equatable {
  ...
  public override var asMarkdown: String {
    let data = try! JSONEncoder().encode(self) // swiftlint:disable:this force_try
  ...
  public override var challenges: [Challenge] {
    var cards: [Challenge] = [
      VocabularyAssociationCard(vocabularyAssociation: self, promptWithSpanish: true, templateIndex: 0),
      VocabularyAssociationCard(vocabularyAssociation: self, promptWithSpanish: false, templateIndex: 1),
    ]
    if testSpelling {
      cards.append(VocabularyAssociationSpellingCard(vocabularyAssociation: self, templateIndex: 2))
    }
    return cards
  }
}
