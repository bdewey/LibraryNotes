  ...
  func testRemoteStudyOfLocalPageGetsIncorporated() async throws {
    try await MergeTestCase()
      ...
      .performRemoteModification { storage in
        ...
        var studySession = try await storage.studySession(date: future)
      ...
      .validate { storage in
        XCTAssertTrue(storage.hasUnsavedChanges)
        let future = Date().addingTimeInterval(5 * 24 * 60 * 60)
        let studySession = try await storage.studySession(date: future)
  ...
  func testRemoteStudyOfRemotePageGetsIncorporated() async throws {
    try await MergeTestCase()
      ...
      .validate { storage in
        ...
        let futureStudySession = try await storage.studySession(date: future.addingTimeInterval(30 * 24 * 60 * 60))
...
extension NoteDatabase {
  func studySession(noteIdentifiers: Set<Note.Identifier>? = nil, date: Date) async throws -> StudySession {
    let sessionGenerator = SessionGenerator(database: self)
    try await sessionGenerator.startMonitoringDatabase()
    return try await sessionGenerator.studySession(noteIdentifiers: noteIdentifiers, date: date)
  }
}
